...
class TradeSignal:
    ...
    def positionDetail(self):
        db = Session()
        self.PositionList = db.query(Position).filter_by(code=self.code).all()
    ...
    def alert1(self, _indata):  # 冲击均线策略
        ...
        try:
            ...
            if np.all([_indata.price - self.KAMASeries[-1] < -0.2, _indata.price >= current_KAMA - 0.1,
                ...
                if self.PositionList.__len__() <= 10:
    ...
    def tradeSignalCallback(self, indatas):
        if 'RT_LAST' in indatas.Fields:
            ...
            if indata.price > 80:  # 筛除异常数据，有些异常值是价格的变动值，在0附近，选择大于80，可以过滤掉这种情况
                self.addSeq(indata)

    def start(self):
        w.start()
        w.wsq(self.code, "rt_date,rt_time,rt_pre_close,rt_open,rt_high,rt_low,rt_last,rt_last_amt,rt_last_vol,rt_ask1,rt_bid1", func=self.tradeSignalCallback)
        while 1:
            self.positionDetail()
            print('持仓列表更新')
...
if __name__ == '__main__':
    ...
    codeList = ['113026.SH', '113011.SH', '113021.SH', '113013.SH']
    ...
    #     indata = InData()
    #     indata.Times = pd.to_datetime("20190528 %s" % df.time[i].strftime("%H:%M:%S"))
