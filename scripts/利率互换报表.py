...
DF2 = pd.DataFrame(columns=[
    ['序号', '交易对手', '本月交易情况', '本月交易情况', '本月交易情况', '本月交易情况', '期末持仓合约', '期末持仓合约', '期末持仓合约', '期末持仓合约', '期末持仓合约', '期末持仓合约',
     '期末持仓合约', '期末持仓合约', '期末持仓合约', '本年累计交易情况', '本年累计交易情况', '本年累计交易情况', '本年累计交易情况'],
    ['', '', '收固定付浮动合约名义本金', '收浮动付固定合约名义本金', '其他形式合约名义本金', '本月已实现损益 注1', '收固定付浮动合约名义本金', '收固定付浮动合约价值', '收浮动付固定合约名义本金',
     '收浮动付固定合约价值', '其他形式合约名义本金', '其他形式合约价值', '已有效对冲风险的合约名义本金 注2', '已有效对冲风险的合约价值 注2', '期末持仓合约未实现损益 注3', '收固定付浮动合约名义本金',
     '收浮动付固定合约名义本金', '其他形式合约名义本金', '本年累计已实现损益 注1']
    ])

DF2['交易对手'] = list(set(dfTradeDetail['对手方'].unique()) | set(dfResidueDetail['对手方'].unique()))
DF2['序号'] = [i + 1 for i in range(DF2['交易对手'].shape[0])]
DF2[('本月交易情况', '收固定付浮动合约名义本金')] = DF2['交易对手'].apply(lambda x: sum(dfTradeDetail[(dfTradeDetail['对手方'] == x) & (
            dfTradeDetail['交易方向'] == '收取固定利率') & (dfTradeDetail['成交时间'] >= CurrentMonth)]['名义本金额(万元)']))*10000
DF2[('本月交易情况', '收浮动付固定合约名义本金')] = DF2['交易对手'].apply(lambda x: sum(dfTradeDetail[(dfTradeDetail['对手方'] == x) & (
            dfTradeDetail['交易方向'] == '支付固定利率') & (dfTradeDetail['成交时间'] >= CurrentMonth)]['名义本金额(万元)']))*10000
DF2[('本月交易情况', '本月已实现损益 注1')] = DF2['交易对手'].apply(
    lambda x: sum(dfOffDetail[(dfOffDetail['对手方'] == x) & (dfOffDetail['操作时间'] >= CurrentMonth)]['冲销估值（元）']))

DF2[('期末持仓合约', '收固定付浮动合约名义本金')] = DF2['交易对手'].apply(
    lambda x: sum(dfResidueDetail[(dfResidueDetail['对手方'] == x) & (dfResidueDetail['交易方向'] == '收取固定')]['剩余名义本金（万元）']))*10000
DF2[('期末持仓合约', '收固定付浮动合约价值')] = DF2['交易对手'].apply(
    lambda x: sum(dfResidueDetail[(dfResidueDetail['对手方'] == x) & (dfResidueDetail['交易方向'] == '收取固定')]['估值（元）（T+1）']))
DF2[('期末持仓合约', '收浮动付固定合约名义本金')] = DF2['交易对手'].apply(
    lambda x: sum(dfResidueDetail[(dfResidueDetail['对手方'] == x) & (dfResidueDetail['交易方向'] == '支付固定')]['剩余名义本金（万元）']))*10000
DF2[('期末持仓合约', '收浮动付固定合约价值')] = DF2['交易对手'].apply(
    lambda x: sum(dfResidueDetail[(dfResidueDetail['对手方'] == x) & (dfResidueDetail['交易方向'] == '支付固定')]['估值（元）（T+1）']))
DF2[('期末持仓合约', '期末持仓合约未实现损益 注3')] = DF2[('期末持仓合约', '收固定付浮动合约价值')] + DF2[('期末持仓合约', '收浮动付固定合约价值')]

DF2[('本年累计交易情况', '收固定付浮动合约名义本金')] = DF2['交易对手'].apply(lambda x: sum(dfTradeDetail[(dfTradeDetail['对手方'] == x) & (
            dfTradeDetail['交易方向'] == '收取固定利率') & (dfTradeDetail['成交时间'] >= CurrentYear)]['名义本金额(万元)']))*10000
DF2[('本年累计交易情况', '收浮动付固定合约名义本金')] = DF2['交易对手'].apply(lambda x: sum(dfTradeDetail[(dfTradeDetail['对手方'] == x) & (
            dfTradeDetail['交易方向'] == '支付固定利率') & (dfTradeDetail['成交时间'] >= CurrentYear)]['名义本金额(万元)']))*10000
DF2[('本年累计交易情况', '本年累计已实现损益 注1')] = DF2['交易对手'].apply(
    lambda x: sum(dfOffDetail[(dfOffDetail['对手方'] == x) & (dfOffDetail['操作时间'] >= CurrentYear)]['冲销估值（元）']))

# DF2[[('对手方',''),('期末持仓合约','期末持仓合约未实现损益 注3')]]

# E1_8_1证券公司参与利率互换交易专项监管报表-明细
DF1 = pd.DataFrame(columns=[
    ['序号', '交易日期', '固定利息支付方', '浮动利息支付方', '名义本金（万元）', '合约期限', '起息日', '到期日', '固定利率', '固定利率', '固定利率', '浮动利率', '浮动利率',
     '浮动利率', '首次利息支付（互换）日'],
...
DF1[('固定利息支付方', '')] = dfResidueDetail[['对手方', '交易方向']].apply(
    lambda row: '东海证券' if row['交易方向'] == '支付固定' else row['对手方'], axis=1)
DF1[('浮动利息支付方', '')] = dfResidueDetail[['对手方', '交易方向']].apply(
    lambda row: row['对手方'] if row['交易方向'] == '支付固定' else '东海证券', axis=1)
...
DF1[('首次利息支付（互换）日', '')] = dfResidueDetail['合约编号'].apply(
    lambda x: dfTradeDetail.loc[dfTradeDetail['成交编号'] == x, '初始支付日-固定利率方'].values[0])
DF1[('交易日期', '')] = dfResidueDetail['合约编号'].apply(
    lambda x: dfTradeDetail.loc[dfTradeDetail['成交编号'] == x, '成交时间'].values[0])

# NAFMII月报-业务统计
NADF1 = pd.DataFrame(columns=['协议类别', '上月末本年累计', '本月新增', '本年累计', '本月末有效'])
NADF1['协议类别'] = ['主协议', '补充协议', '履约保障协议', '交易确认书']
NADF1 = NADF1.fillna(0)
NADF1.loc[0, '上月末本年累计'] = ((CurrentYear <= dfTradeDetail["成交时间"]) & (dfTradeDetail["成交时间"] < CurrentMonth)).sum()
...
# 利率
NADF12.loc[0, ('名义本金（亿元）（小数点后两位小数）', '本月新增交易')] = dfTradeDetail[dfTradeDetail['成交时间'] >= CurrentMonth][
                                                      '名义本金额(万元)'].sum() / 10000  # /10000单位换算
NADF12.loc[0, ('名义本金（亿元）（小数点后两位小数）', '本月了结交易')] = dfOffDetail[dfOffDetail['操作时间'] >= CurrentMonth][
                                                      '原剩余名义本金（万元）'].sum() / 10000
NADF12.loc[0, ('名义本金（亿元）（小数点后两位小数）', '本月末存续交易')] = dfResidueDetail['剩余名义本金（万元）'].sum() / 10000
NADF12.loc[0, ('名义本金（亿元）（小数点后两位小数）', '截至本月末年度新增交易')] = dfTradeDetail[dfTradeDetail['成交时间'] >= CurrentYear][
                                                           '名义本金额(万元)'].sum() / 10000
...
if temp.shape[0] >= 1:
    ...
    NADF2['固定利息支付方（机构名称）'] = temp[['对手方', '交易方向']].apply(lambda row: '东海证券' if row['交易方向'] == '支付固定利率' else row['对手方'],
                                                         axis=1)
    NADF2['浮动利息支付方（机构名称）'] = temp[['对手方', '交易方向']].apply(lambda row: row['对手方'] if row['交易方向'] == '支付固定利率' else '东海证券',
                                                         axis=1)
    NADF2['名义本金（亿元）'] = temp['名义本金额(万元)'] / 10000
    NADF2[('起息日', '')] = temp['起息日']
    NADF2[('到期日', '')] = temp['到期日']
    NADF2[('合约期限（天）', '')] = temp[['起息日', '到期日']].apply(
        lambda row: pd.to_datetime(row['到期日']) - pd.to_datetime(row['起息日']), axis=1).apply(lambda x: x.days)
